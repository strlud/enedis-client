...
const API_HOST = 'https://www.myelectricaldata.fr';
...
export class Session {
  private prms: string[] = [];
  public userAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:147.0) Gecko/20100101 Firefox/147.0';

  constructor(private token: string, private prm?: string) {
    this.prm = prm ?? process.env.PRM;
  ...
  private callApi<T>(type: DataType, start: string, end: string): Promise<T> {
    ...
    return axios
      ...
      .then((res) => (res.data as any).meter_reading)
      .catch((err) => {
        if (err.response) {
